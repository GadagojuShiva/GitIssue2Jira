import requests
from requests.auth import HTTPBasicAuth
import json
from flask import Flask, request

app = Flask(__name__)

@app.route('/createJIRA', methods=['POST'])
def createJIRA():
    # Extract JSON payload from GitHub webhook
    github_payload = request.json
    
    # Check if the payload contains a comment
    if 'comment' in github_payload:
        comment_body = github_payload['comment']['body']
        # Check if "/jira" is mentioned in the comment
        if "/jira" in comment_body.lower():
            url = "https://xxxxxx.atlassian.net/rest/api/3/issue"

            API_TOKEN = "YOUR_JIRA_API_TOKEN"
            auth = HTTPBasicAuth("xxxx@gmail.com", API_TOKEN)

            headers = {
                "Accept": "application/json",
                "Content-Type": "application/json"
            }

            payload = json.dumps({
                "fields": {
                    "description": {
                        "content": [
                            {
                                "content": [
                                    {
                                        "text": "Order entry fails when selecting supplier.",
                                        "type": "text"
                                    }
                                ],
                                "type": "paragraph"
                            }
                        ],
                        "type": "doc",
                        "version": 1
                    },
                    "project": {
                        "key": "GIT"
                    },
                    "issuetype": {
                        "id": "10001"
                    },
                    "summary": "Main order flow broken",
                },
                "update": {}
            })

            response = requests.request(
                "POST",
                url,
                data=payload,
                headers=headers,
                auth=auth
            )

            return json.dumps(json.loads(response.text), sort_keys=True, indent=4, separators=(",", ": "))

    return "No JIRA ticket created."

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000)
